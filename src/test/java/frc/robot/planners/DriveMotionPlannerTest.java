...
public class DriveMotionPlannerTest {
  ...
  public void testFollowerReachesGoal() {
    ...
    motion_planner.setTrajectory(
        new TrajectoryIterator<>(new TimedView<>(motion_planner.generateTrajectory(false, Arrays
                .asList(new Pose2d(new Translation2d(0.0, 0.0), Rotation2d.identity()),
                    new Pose2d(new Translation2d(120.0, -36.0), Rotation2d.identity()),
                    new Pose2d(new Translation2d(240.0, -36.0), Rotation2d.identity())), null, 120.0,
            120.0, 10.0))));
    ...
    while (!motion_planner.isDone()) {
      ...
      Twist2d delta = Kinematics
          .forwardKinematics(output.left_velocity * dt * Constants.DRIVE.kWheelDiameter / 2.0,
              output.right_velocity * dt * Constants.DRIVE.kWheelDiameter / 2.0);
  ...
  public void testVoltages() {
    ...
    motion_planner.setTrajectory(
        new TrajectoryIterator<>(new TimedView<>(motion_planner.generateTrajectory(false, Arrays
                .asList(Pose2d.identity(), Pose2d.fromTranslation(new Translation2d(48.0, 0.0)),
