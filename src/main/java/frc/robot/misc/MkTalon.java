...
public class MkTalon {
     ...
     *
     * <p>
     * All of the persistent configs are stored in {@link Constants.CONFIG}. These primarily
     * configure PID/Limit Switch/Encoder settings.
     *
     * <p>
     * Errors will appear on the Driver Station and will be logged to disk if a config() method
     * return an error.
     */
    public synchronized void resetConfig() {
        ...
        switch (mSide) {
            case Right:
                ...
                CTRE(
                    masterTalon.setStatusFramePeriod(StatusFrame.Status_13_Base_PIDF0, 10, kShort));
                CTRE(
                    masterTalon.setStatusFramePeriod(StatusFrame.Status_14_Turn_PIDF1, 10, kShort));
            ...
            case Left:
                ...
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_1_General, 5, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_2_Feedback0, 5, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_3_Quadrature, 100, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_8_PulseWidth, 100, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_10_MotionMagic, 10, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_13_Base_PIDF0, 10, kShort));
                zeroEncoder();
                break;
            case Cargo_Arm:
            case Hatch_Arm:
                CTRE(masterTalon.setControlFramePeriod(ControlFrame.Control_3_General, 5));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_1_General, 10, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_2_Feedback0, 20, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_3_Quadrature, 100, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_8_PulseWidth, 10, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_10_MotionMagic, 50, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_13_Base_PIDF0, 50, kShort));
                zeroEncoder();
                break;
            case Cargo_Intake:
                CTRE(masterTalon.setControlFramePeriod(ControlFrame.Control_3_General, 5));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_1_General, 5, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_2_Feedback0, 100, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_3_Quadrature, 100, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_8_PulseWidth, 100, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_10_MotionMagic, 100, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_13_Base_PIDF0, 100, kShort));
                CTRE(masterTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_11_UartGadgeteer, 5, kShort));
                break;
            default:
                Logger.logError("Unknown Side");
                break;
        ...
        if (mSide == TalonLoc.Hatch_Arm || mSide == TalonLoc.Cargo_Intake) {
            ...
            if (mSide == TalonLoc.Hatch_Arm) {
                ...
                CTRE(slaveTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_1_General, 3, kShort));
                CTRE(slaveTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_2_Feedback0, 100, kShort));
                CTRE(slaveTalon
                    .setStatusFramePeriod(StatusFrameEnhanced.Status_3_Quadrature, 100, kShort));
        ...
        if (mSide == TalonLoc.Cargo_Intake) {
            ...
            CTRE(
                mPigeon.setStatusFramePeriod(PigeonIMU_StatusFrame.CondStatus_1_General, 3, kLong));
            CTRE(mPigeon
                .setStatusFramePeriod(PigeonIMU_StatusFrame.CondStatus_9_SixDeg_YPR, 3, kLong));
            CTRE(mPigeon
                .setStatusFramePeriod(PigeonIMU_StatusFrame.CondStatus_6_SensorFusion, 3, kLong));
        }
     ...
     * @param arbFeed Arbitrary feedforward added as a PercentOutput to any closed loop (or open
     * loop) mode.
     */
    public synchronized void set(ControlMode mode, double value, DemandType type, double arbFeed,
        NeutralMode nMode) {
        ...
        if (mode != lastControlMode || value != lastOutput || lastDemandType != type
            || arbFeed != lastArbFeed || motorSafetyTimer.isDone()) {
    ...
    public synchronized void zeroEncoder() {
        if (mSide == TalonLoc.Left || mSide == TalonLoc.Right) {
            ...
            CTRE(masterTalon.getSensorCollection()
                .syncQuadratureWithPulseWidth(CARGO_ARM.kBookEnd_0, CARGO_ARM.kBookEnd_1,
                    CARGO_ARM.kCrossOverZero, CARGO_ARM.kOffset, kShort));
        } else if (mSide == TalonLoc.Hatch_Arm) {
            CTRE(masterTalon.getSensorCollection()
                .syncQuadratureWithPulseWidth(HATCH_ARM.kBookEnd_0, HATCH_ARM.kBookEnd_1,
    ...
    public synchronized double getVelocity() {
        switch (mSide) {
            case Cargo_Arm:
            case Hatch_Arm:
                return MkMath.nativeUnitsPer100MstoDegreesPerSec(
                    masterTalon.getSelectedSensorVelocity(CONFIG.kPIDPrimary));
            case Left:
            case Right:
    ...
    public synchronized double getPosition() {
        switch (mSide) {
            case Cargo_Arm:
            case Hatch_Arm:
                return MkMath.nativeUnitsToDegrees(
                    masterTalon.getSelectedSensorPosition(CONFIG.kPIDPrimary));
            case Left:
            case Right:
                return MkMath
                    .nativeUnitsToInches(masterTalon.getSelectedSensorPosition(CONFIG.kPIDPrimary));
            default:
                Logger.logErrorWithTrace("Talon doesn't have encoder");
                return 0.0;
        }
     ...
     *
     * <p>
     * The cargo and ground intake move to each available setpoint and should be verified by the
     * test operator.
     *
        ...
        switch (mSide) {
            ...
            case Right:
                ...
                if (mPos < TEST.kMinDriveTestPos || mVel < TEST.kMinDriveTestVel) {
                    Logger.logErrorWithTrace("FAILED - " + mSide.toString()
                        + " Slave FAILED TO REACH REQUIRED SPEED OR POSITION");
                    Logger.logMarker(
            ...
            case Cargo_Arm:
                 ...
                 *
                 * masterTalon.set(ControlMode.PercentOutput, 0.0);
                 * slaveVictor.set(ControlMode.PercentOutput, 0.0); time.reset();
                 *
                 ...
                 * "Cargo Arm Slave FAILED TO REACH REQUIRED SPEED OR POSITION");
                 * Logger.logMarker(mSide.toString() + " Slave Test Failed - Vel: " + vel + " Pos: " + pos +
                 * " Current: " + current); check = false; } else { Logger.logMarker(mSide.toString() +
                 * " Slave - Vel: " + vel + " Pos: " + pos + " Current: " + current); }
                 *
                 * Timer.delay(1.0);
                 *
                 ...
                 * TEST.kMinCargoArmTestPos) { Logger.logErrorWithTrace("FAILED - " + mSide.toString() +
                 * "Cargo Arm Slave FAILED TO REACH REQUIRED SPEED OR POSITION");
                 * Logger.logMarker(mSide.toString() + " Master Test Failed - Vel: " + vel + " Pos: " +
                 * pos); check = false; } else { Logger.logMarker(mSide.toString() + " Master - Vel: " + vel
                 * + " Pos: " + pos); }
                 *
            ...
            case Hatch_Arm:
                ...
                /*
                 * Timer.delay(1.0); masterTalon.set(ControlMode.PercentOutput, -0.3); Timer.delay(1.0);
                 * current = getCurrent(); vel = getVelocity(); pos = getPosition(); if (vel <
                 * TEST.kHatchArmVel || current > TEST.kHatchArmCurrent || pos < TEST.kHatchArmPos) {
                 * Logger.logErrorWithTrace("FAILED - " + mSide.toString() +
                 * "Hatch FAILED TO REACH REQUIRED SPEED OR POSITION"); Logger.logMarker(mSide.toString() +
                 * " Test Failed - Vel: " + vel + " Pos: " + pos + " Current: " + current); check = false; }
                 * else { Logger.logMarker(mSide.toString() + " - Vel: " + vel + " Pos: " + pos +
                 * " Current: " + current); }
                 */
                break;
            case Cargo_Intake:
    ...
    /**
     * Logs and sends error to DS if any Phoenix Config method returns an Error Code that is not
     * 'OK'
     */
    ...
    @Override public String toString() {
        return "Output: " + masterTalon.getMotorOutputPercent() + " Current: " + masterTalon
            .getOutputCurrent() + (mSide != TalonLoc.Cargo_Intake ? " Pos: " + getPosition()
            + " Vel: " + getVelocity() : " No Encoder");
    }

    /**
     * Left Drive and Right Drive house the Talon, Victor, and SRX Mag encoder for each side of the
     * drivetrain. The hatch Arm houses the ground hatch intake Talon, SRX Mag Encoder, and an
     * unused Talon with a Breakout board with two limit switches. One limit switch is placed at the
     * reverse hardstop for the ground intake arm, and the second is placed on the pneumatic spear
     * arm to detect when the main arm is inside the target. The Cargo Arm houses the Talon, Victor,
