...
public class MkTalon {
  ...
  public synchronized void resetConfig() {
    ...
    switch (mSide) {
      ...
      case Left:
        ...
        CTRE(
            masterTalon.setStatusFramePeriod(StatusFrameEnhanced.Status_3_Quadrature, 100, kShort));
        CTRE(
            masterTalon.setStatusFramePeriod(StatusFrameEnhanced.Status_8_PulseWidth, 100, kShort));
        CTRE(masterTalon.setStatusFramePeriod(StatusFrameEnhanced.Status_10_MotionMagic, 10,
            kShort));
        CTRE(
            masterTalon.setStatusFramePeriod(StatusFrameEnhanced.Status_13_Base_PIDF0, 10, kShort));
        zeroEncoder();
        break;
      case Cargo_Arm:
      case Hatch_Arm:
        ...
        CTRE(masterTalon.setStatusFramePeriod(StatusFrameEnhanced.Status_10_MotionMagic, 50,
            kShort));
        CTRE(
            masterTalon.setStatusFramePeriod(StatusFrameEnhanced.Status_13_Base_PIDF0, 50, kShort));
        zeroEncoder();
        break;
      case Cargo_Intake:
        CTRE(masterTalon.setControlFramePeriod(ControlFrame.Control_3_General, 5));
        ...
        CTRE(
            masterTalon.setStatusFramePeriod(StatusFrameEnhanced.Status_8_PulseWidth, 100, kShort));
        CTRE(masterTalon.setStatusFramePeriod(StatusFrameEnhanced.Status_10_MotionMagic, 100,
            kShort));
        CTRE(masterTalon.setStatusFramePeriod(StatusFrameEnhanced.Status_13_Base_PIDF0, 100,
            kShort));
        CTRE(masterTalon.setStatusFramePeriod(StatusFrameEnhanced.Status_11_UartGadgeteer, 5,
            kShort));
        break;
      default:
        Logger.logError("Unknown Side");
    ...
    if (mSide == TalonLoc.Hatch_Arm || mSide == TalonLoc.Cargo_Intake) {
      ...
      if (mSide == TalonLoc.Hatch_Arm) {
        ...
        CTRE(slaveTalon.setStatusFramePeriod(StatusFrameEnhanced.Status_10_MotionMagic, 100,
            kShort));
        CTRE(
            slaveTalon.setStatusFramePeriod(StatusFrameEnhanced.Status_13_Base_PIDF0, 100, kShort));
  ...
  public synchronized void zeroEncoder() {
    if (mSide == TalonLoc.Left || mSide == TalonLoc.Right) {
      ...
      CTRE(masterTalon.getSensorCollection().syncQuadratureWithPulseWidth(CARGO_ARM.kBookEnd_0,
          CARGO_ARM.kBookEnd_1, CARGO_ARM.kCrossOverZero, CARGO_ARM.kOffset, kShort));
    } else if (mSide == TalonLoc.Hatch_Arm) {
      CTRE(masterTalon.getSensorCollection().syncQuadratureWithPulseWidth(HATCH_ARM.kBookEnd_0,
          HATCH_ARM.kBookEnd_1, HATCH_ARM.kCrossOverZero, HATCH_ARM.kOffset, kShort));
    } else {
  ...
  public synchronized double getPosition() {
    switch (mSide) {
      ...
      case Hatch_Arm:
        return MkMath
            .nativeUnitsToDegrees(masterTalon.getSelectedSensorPosition(CONFIG.kPIDPrimary));
      case Left:
      case Right:
        return MkMath
   ...
   *         {@link BaseMotorController#getClosedLoopError()} method. This method serves to limit
    ...
    switch (mSide) {
      ...
      case Right:
        ...
        if (mPos < TEST.kMinDriveTestPos || mVel < TEST.kMinDriveTestVel) {
          ...
          Logger
              .logMarker(mSide.toString() + " Slave Test Failed - Vel: " + mVel + " Pos: " + mPos);
          check = false;
        } else {
        ...
        if (mPos < TEST.kMinDriveTestPos || mVel < TEST.kMinDriveTestVel) {
          ...
          Logger
              .logMarker(mSide.toString() + " Master Test Failed - Vel: " + mVel + " Pos: " + mPos);
          check = false;
        } else {
  ...
  public String toString() {
    return "Output: " + masterTalon.getMotorOutputPercent() + " Current: "
        + masterTalon.getOutputCurrent()
        + (mSide != TalonLoc.Cargo_Intake ? " Pos: " + getPosition() + " Vel: " + getVelocity()
            : " No Encoder");
  }
