...
public class MotionMagicVisionPigeon implements Action {
    ...
    /**
     * Exit tracking if it is taking too long or if the tracking is finished or if the arm is down
     * for a specified time and the limit switch is triggered.
     */
    @Override public boolean isFinished() {
        return timer.isDone() || Drive.getInstance()
            .isVisionFinished(lastDist, lastAngle + lastSkew) || (
            HatchArm.getInstance().isHatchLimitTriggered() && (
                (HatchArm.getInstance().getHatchSpearState() == HatchSpearState.PLACE) && downTimer
                    .isDone()));
    }
        ...
        switch (goal) {
            ...
            case PLACE_HATCH:
                if ((Drive.getInstance().getVisionServoError(lastDist) < 28.0) && (
                    ...
                    HatchArm.getInstance()
                        .setHatchMechanismState(HatchMechanismState.SPEAR_PLACE_ONLY);
            ...
            case INTAKE_HATCH:
                if ((Drive.getInstance().getVisionServoError(lastDist) < 60.0) && (
                    HatchArm.getInstance().getHatchSpearState() != HatchSpearState.PLACE)) {
                    HatchArm.getInstance()
                        .setHatchMechanismState(HatchMechanismState.STATION_INTAKE);
                    downTimer.start(1.0);
                }
                break;
            default:
