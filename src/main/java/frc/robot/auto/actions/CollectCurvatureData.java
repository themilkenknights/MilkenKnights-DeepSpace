...
public class CollectCurvatureData implements Action {
	 ...
	 */
	public CollectCurvatureData(List<DriveCharacterization.CurvatureDataPoint> data, boolean highGear,
			boolean reverse) {
		mCurvatureData = data;
		mReverse = reverse;
		mCSVWriter = new ReflectingCSVWriter<>("CURVATURE_DATA.csv",
				DriveCharacterization.CurvatureDataPoint.class);
	}
	...
	public void update() {
		...
		mDrive.setOpenLoop(new DriveSignal((mReverse ? -1.0 : 1.0) * kStartPower,
				(mReverse ? -1.0 : 1.0) * rightPower));
		mCurvatureData.add(
				new DriveCharacterization.CurvatureDataPoint(mRobotState.getPredictedVelocity().dx,
