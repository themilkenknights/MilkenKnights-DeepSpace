...
public class CollectCurvatureData implements Action {
	...
	public void update() {
		...
		if (rightPower > kMaxPower) { isFinished = true; return; }
	...
	@Override
	public void done() { mDrive.setOpenLoop(DriveSignal.BRAKE); mCSVWriter.flush(); }

	@Override
	public void start() { mDrive.setOpenLoop(new DriveSignal(kStartPower, kStartPower)); mStartTime = Timer.getFPGATimestamp(); }
}
