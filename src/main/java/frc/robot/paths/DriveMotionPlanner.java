...
public class DriveMotionPlanner implements CSVWritable {
  ...
  public DriveMotionPlanner() {
    final DCMotorTransmission transmission = new DCMotorTransmission(1.0 / Constants.DRIVE.kDriveKv,
        Units.inches_to_meters(Constants.DRIVE.kDriveWheelRadiusInches) * Units
            .inches_to_meters(Constants.DRIVE.kDriveWheelRadiusInches)
            * Constants.DRIVE.kRobotLinearInertia / (2.0 * Constants.DRIVE.kDriveKa),
        Constants.DRIVE.kDriveVIntercept);
    mModel = new DifferentialDrive(Constants.DRIVE.kRobotLinearInertia,
        Constants.DRIVE.kRobotAngularInertia, Constants.DRIVE.kRobotAngularDrag,
        Units.inches_to_meters(Constants.DRIVE.kWheelDiameter / 2.0),
        Units.inches_to_meters(
            Constants.DRIVE.kDriveWheelTrackWidthInches / 2.0 * Constants.DRIVE.kTrackScrubFactor),
  ...
  public Trajectory<TimedState<Pose2dWithCurvature>> generateTrajectory(boolean reversed,
      ...
      double max_voltage) {
    return generateTrajectory(reversed, waypoints, constraints, 0.0, 0.0, max_vel, max_accel,
        max_voltage);
  }

  public Trajectory<TimedState<Pose2dWithCurvature>> generateTrajectory(boolean reversed,
      final List<Pose2d> waypoints,
      final List<TimingConstraint<Pose2dWithCurvature>> constraints, double start_vel,
      double end_vel, double max_vel,  // inches/s
      double max_accel,  // inches/s^2
      double max_voltage) {
    ...
    Trajectory<TimedState<Pose2dWithCurvature>> timed_trajectory = TimingUtil
        .timeParameterizeTrajectory(reversed, new DistanceView<>(trajectory), kMaxDx,
            all_constraints, start_vel, end_vel, max_vel, max_accel);
  ...
  public String toCSV() {
    ...
    return fmt.format(mOutput.left_velocity) + "," + fmt.format(mOutput.right_velocity) + "," + fmt
        .format(mOutput.left_feedforward_voltage) + ","
        + fmt.format(mOutput.right_feedforward_voltage) + "," + mSetpoint.toCSV();
  }
  ...
  public Output update(double timestamp, Pose2d current_state) {
    ...
    if (!mCurrentTrajectory.isDone()) {
      ...
      final DifferentialDrive.DriveDynamics dynamics = mModel
  ...
  protected Output updatePurePursuit(DifferentialDrive.DriveDynamics dynamics,
    ...
    while (actual_lookahead_distance < Constants.DRIVE.kPathMinLookaheadDistance
    ...
    if (actual_lookahead_distance < Constants.DRIVE.kPathMinLookaheadDistance) {
      lookahead_state = new TimedState<>(new Pose2dWithCurvature(
          lookahead_state.state().getPose().transformBy(Pose2d.fromTranslation(
              new Translation2d(
                  (mIsReversed ? -1.0 : 1.0) * (Constants.DRIVE.kPathMinLookaheadDistance
                      - actual_lookahead_distance), 0.0))), 0.0),
    ...
    adjusted_velocity.linear = dynamics.chassis_velocity.linear + Constants.DRIVE.kPathKX * Units
        .inches_to_meters(mError.getTranslation().x());
    ...
    final PurePursuitController.Arc<Pose2dWithCurvature> arc = new PurePursuitController.Arc<>(
        current_state, lookahead_state.state());
  ...
  protected Output updateNonlinearFeedback(DifferentialDrive.DriveDynamics dynamics,
    ...
    final double k = 2.0 * kZeta * Math
        .sqrt(kBeta * dynamics.chassis_velocity.linear * dynamics.chassis_velocity.linear
    ...
    final DifferentialDrive.ChassisState adjusted_velocity = new DifferentialDrive.ChassisState(
        dynamics.chassis_velocity.linear * mError.getRotation().cos() + k * Units
            .inches_to_meters(mError.getTranslation().x()),
        dynamics.chassis_velocity.angular + k * angle_error_rads
            + dynamics.chassis_velocity.linear * kBeta * sin_x_over_x * Units
    ...
    DifferentialDrive.WheelState feedforward_voltages =
        mModel
            .solveInverseDynamics(dynamics.chassis_velocity, dynamics.chassis_acceleration).voltage;
    return new Output(dynamics.wheel_velocity.left, dynamics.wheel_velocity.right,
        dynamics.wheel_acceleration.left,
  ...
  public static class Output {
    ...
    public Output(double left_velocity, double right_velocity, double left_accel,
        double right_accel, double left_feedforward_voltage,
