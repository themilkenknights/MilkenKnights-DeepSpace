...
public class DriveMotionPlanner implements CSVWritable {
	...
	public DriveMotionPlanner() {
		final DCMotorTransmission transmission = new DCMotorTransmission(
				1.0 / Constants.DRIVE.kDriveKv,
				Units.inches_to_meters(Constants.DRIVE.kDriveWheelRadiusInches) * Units.inches_to_meters(Constants
						.DRIVE.kDriveWheelRadiusInches) * Constants.DRIVE.kRobotLinearInertia / (2.0 * Constants.DRIVE.kDriveAngularKa),
				Constants.DRIVE.kDriveVIntercept);
		mModel = new DifferentialDrive(
				Constants.DRIVE.kRobotLinearInertia,
				Constants.DRIVE.kRobotAngularInertia,
				Constants.DRIVE.kRobotAngularDrag,
				Units.inches_to_meters(Constants.DRIVE.kWheelDiameter / 2.0),
				Units.inches_to_meters(Constants.DRIVE.kEffectiveDriveWheelTrackWidthInches / 2.0 * Constants.DRIVE.kTrackScrubFactor),
	...
	public Trajectory<TimedState<Pose2dWithCurvature>> generateTrajectory(
		...
		// re-architect the spline generator to support reverse.
		if (reversed) {
	...
	protected Output updatePurePursuit(DifferentialDrive.DriveDynamics dynamics, Pose2d current_state) {
		...
		DifferentialDrive.ChassisState adjusted_velocity = new DifferentialDrive.ChassisState();
		// Feedback on longitudinal error (distance).
		adjusted_velocity.linear = dynamics.chassis_velocity.linear + Constants.DRIVE.kPathKX * Units.inches_to_meters
				(mError.getTranslation().x());
