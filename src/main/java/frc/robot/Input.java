...
public class Input {
    ...
    public static void updateControlInput() {
        ...
        boolean isVisionState = currentRobotState == RobotState.VISION_CARGO_OUTTAKE
            || currentRobotState == RobotState.HATCH_VISION_INTAKE
            || currentRobotState == RobotState.HATCH_VISION_OUTTAKE
            || currentRobotState == RobotState.AUTO_CLIMB
        ...
        if (mCargoArmManual.isHeld()) {
            ...
            mHatch.setOpenLoop(
                MkMath.handleDeadband(Math.pow(movement, 3), GENERAL.kOperatorDeadband));
        ...
        if (mAutoClimb.isPressed()) {
            ...
            mStructure.setFrontClimbState(mStructure.getFrontClimbState() == ClimbState.RETRACTED
                ? ClimbState.LOWERED
                : ClimbState.RETRACTED);
        } else if (mRearClimb.isPressed()) {
            // Toggle Rear Climb Actuators (Toward Cargo Arm/Rio)
            mStructure.setRearClimbState(mStructure.getRearClimbState() == ClimbState.RETRACTED
                ? ClimbState.LOWERED
                : ClimbState.RETRACTED);
        }
