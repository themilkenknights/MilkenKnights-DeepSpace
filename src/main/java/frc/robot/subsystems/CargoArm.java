...
public class CargoArm extends Subsystem {
    ...
    private CargoArm() {
        ...
        mArmTalon =
            new MkTalon(CAN.kMasterCargoArmTalonID, CAN.kSlaveCargoArmVictorID, TalonLoc.Cargo_Arm,
                mCargoArmTab);
        mIntakeTalon = new MkTalon(CAN.kLeftCargoIntakeTalonID, CAN.kRightCargoIntakeTalonID,
            TalonLoc.Cargo_Intake, mIntakeRollersTab);
    }
     ...
     *
     * <p>
     * This method also checks for unsafe current output and will automatically switches to open
     * loop control.
     */
    ...
    public synchronized void onQuickLoop(double timestamp) {
        if (mCargoArmControlState == CargoArmControlState.OPEN_LOOP) {
            ...
            if (mCargoArmState.equals(CargoArmState.ENABLE)) {
                mArmTalon.set(ControlMode.MotionMagic, MkMath.degreesToNativeUnits(mArmPosEnable),
                    NeutralMode.Brake);
            } else {
                double armFeed = MkMath.sin(mArmTalon.getPosition() + CARGO_ARM.kArmOffset)
                    * CARGO_ARM.kFeedConstant;
                mArmTalon
                    .set(ControlMode.MotionMagic, MkMath.degreesToNativeUnits(mCargoArmState.state),
                        DemandType.ArbitraryFeedForward, -armFeed, NeutralMode.Brake);
            }
        } else {
            Logger
                .logErrorWithTrace("Unexpected Cargo Arm Control State: " + mCargoArmControlState);
        }
    ...
    public enum CargoArmState {
        ...
        INTAKE(187.0), FORWARD_ROCKET_LEVEL_ONE(120.0), FORWARD_ROCKET_LEVEL_TWO(
            75.0), REVERSE_CARGOSHIP(21.0);
