...
public class HatchArm extends Subsystem {
    ...
    private static mKetteringControlState mHatchIntakeControlState =
        mKetteringControlState.MOTION_MAGIC;
    ...
    private NetworkTableEntry mAbsPos, mDesiredState, mControlMode, mStatus, mRawError, mMechState,
        mLimitTriggered, mSpearStateTab, mRawPos;
    ...
    private HatchArm() {
        ...
        mArmTalon = new MkTalon(CAN.kGroundHatchArmTalonID, CAN.kKetteringReverseLimitSwitchTalonID,
            TalonLoc.Hatch_Arm, mHatchArmTab);
    ...
    @Override public synchronized void onQuickLoop(double timestamp) {
        ...
        switch (mHatchMechanismState) {
            ...
            case TRANSFER:
                if (mMoveTime.isDone() && second) {
                    ...
                    mArmTalon.masterTalon
                        .configMotionCruiseVelocity((int) HATCH_ARM.kMotionMagicCruiseVel);
                    setHatchSpearState(HatchSpearState.STOW);
            ...
            default:
                Logger.logErrorWithTrace(
                    "Unexpected Hatch Arm control state: " + mHatchMechanismState);
                break;
        }
        /*
         ...
         * else if (mHatchIntakeControlState == mKetteringControlState.MOTION_MAGIC) { if
         * (mHatchKetteringSetpoint == mKetteringSetpoint.ENABLE) {
         * mArmTalon.set(ControlMode.MotionMagic, MkMath.degreesToNativeUnits(mKetteringPosEnable),
         * NeutralMode.Brake); } else { mArmTalon.set(ControlMode.MotionMagic,
         * MkMath.degreesToNativeUnits(mHatchKetteringSetpoint.state), NeutralMode.Brake); } } else {
    ...
    public void setHatchMechanismState(HatchMechanismState state) {
        if (mHatchMechanismState == HatchMechanismState.TRANSFER
            && state != HatchMechanismState.TRANSFER) {
            ...
            mArmTalon.masterTalon
                .configMotionCruiseVelocity((int) HATCH_ARM.kMotionMagicCruiseVel / 9);
        }
    ...
    /**
     * The state of the pneumatic spear that places and intakes the Hatches. The default state
     * should always be stowed on power off.
     */
    public enum HatchSpearState {PLACE(HATCH_ARM.kHatchArmPlaceState), STOW(
        !HATCH_ARM.kHatchArmPlaceState);
