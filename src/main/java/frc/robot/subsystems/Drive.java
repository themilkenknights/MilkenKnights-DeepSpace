...
public class Drive extends Subsystem {
    ...
    private Drive() {
        ...
        mLeftDrive = new MkTalon(Constants.CAN.kDriveLeftMasterTalonID,
            Constants.CAN.kDriveLeftSlaveVictorID, TalonLoc.Left, mDriveTab);
        mRightDrive = new MkTalon(Constants.CAN.kDriveRightMasterTalonID,
            Constants.CAN.kDriveRightSlaveVictorID, TalonLoc.Right, mDriveTab);
    ...
    @Override public synchronized void writePeriodicOutputs(double timestamp) {
        if (mDriveControlState == DriveControlState.OPEN_LOOP) {
            mLeftDrive
                .set(ControlMode.PercentOutput, mPeriodicIO.left_demand, mPeriodicIO.brake_mode);
            mRightDrive
                .set(ControlMode.PercentOutput, mPeriodicIO.right_demand, mPeriodicIO.brake_mode);
        } else if (mDriveControlState == DriveControlState.MOTION_MAGIC) {
            mLeftDrive.set(ControlMode.MotionMagic, mPeriodicIO.left_demand,
                DemandType.ArbitraryFeedForward, mPeriodicIO.left_feedforward,
                mPeriodicIO.brake_mode);
            mRightDrive.set(ControlMode.MotionMagic, mPeriodicIO.right_demand,
                DemandType.ArbitraryFeedForward, mPeriodicIO.right_feedforward,
                mPeriodicIO.brake_mode);
        } else if (mDriveControlState == DriveControlState.PIGEON_SERVO) {
            mLeftDrive.set(ControlMode.MotionMagic, mPeriodicIO.left_demand, DemandType.AuxPID,
                mPeriodicIO.left_feedforward, mPeriodicIO.brake_mode);
            mRightDrive.set(ControlMode.MotionMagic, mPeriodicIO.right_demand, DemandType.AuxPID,
                mPeriodicIO.right_feedforward, mPeriodicIO.brake_mode);
        } else if (mDriveControlState == DriveControlState.VELOCITY_SETPOINT) {
            mLeftDrive
                .set(ControlMode.MotionMagic, mPeriodicIO.left_demand, mPeriodicIO.brake_mode);
            mRightDrive
                .set(ControlMode.MotionMagic, mPeriodicIO.right_demand, mPeriodicIO.brake_mode);
        } else {
            Logger.logErrorWithTrace("Unexpected drive control state: " + mDriveControlState);
        }
    }
    ...
    /**
     * Controls Drivetrain in Closed-loop velocity Mode Method sets Talons in Native Units per
     * 100ms
     *
     ...
     */
    public synchronized void updateMotionMagicPositionSetpoint(DriveSignal signal,
    ...
    public synchronized DriveSignal setMotionMagicDeltaSetpoint(DriveSignal signal,
        DriveSignal feedforward) {
    ...
    /**
     * Use Limelight to find Distance and Angle. Drive and turn to target using primary and aux PID
     * on Talons
     *
    ...
    public void setVelocitySetpointNormal(DriveSignal sig) {
        setVelocity(new DriveSignal(sig.getLeft() * DRIVE.kMaxNativeVel,
            sig.getRight() * DRIVE.kMaxNativeVel), DriveSignal.BRAKE);
    }
