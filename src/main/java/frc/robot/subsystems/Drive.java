...
import frc.robot.util.auto.trajectory.PathFollower;
...
public class Drive extends Subsystem {
    ...
    public synchronized void setDrivePath(Path path, double dist_tol, double ang_tol, boolean brakeMode) {
        ...
        brakePath = brakeMode;
        double offset = lastAngle
                - Pathfinder.boundHalfDegrees(Pathfinder.r2d(path.getLeftWheelTrajectory().get(0).heading));
    ...
    /*
     * Called from Auto Action to check when the path finishes. Saves the last angle
     * to use with the next path and resets the Trajectory Status
     */
    ...
    /**
     * Called from Looper during Path Following Gets a TrajectoryStatus containing
     * output velocity and Desired Trajectory Information for logging Inputs
     * Position, Speed and Angle to Trajectory Follower Creates a new Drive Signal
     * that is then set as a velocity setpoint
     */
    private synchronized void updatePathFollower() {
        TrajectoryStatus leftUpdate = pathFollower.getLeftVelocity(leftDrive.getPosition(), leftDrive.getSpeed(),
                -navX.getYaw());
        TrajectoryStatus rightUpdate = pathFollower.getRightVelocity(rightDrive.getPosition(), rightDrive.getSpeed(),
                -navX.getYaw());
        leftStatus = leftUpdate;
        rightStatus = rightUpdate;
        if (isEncodersConnected()) {
            setVelocitySetpoint(new DriveSignal(leftUpdate.getOutput(), rightUpdate.getOutput(), brakePath),
    ...
    public void outputToSmartDashboard() {
         ...
         * SmartDashboard.putNumber("Right Desired Velocity",
         * currentSetpoint.getRight()); SmartDashboard.putNumber("Desired Heading",
         * leftStatus.getSeg().heading); SmartDashboard.putNumber("Heading Error",
         * leftStatus.getAngError()); SmartDashboard.putNumber("Left Desired Position",
         * leftStatus.getSeg().position);
         * SmartDashboard.putNumber("Left Theoretical Vel",
         * leftStatus.getSeg().velocity);
         * SmartDashboard.putNumber("Left Position Error", leftStatus.getPosError());
         * SmartDashboard.putNumber("Left Desired Velocity Error",
         * leftStatus.getVelError()); SmartDashboard.putNumber("Right Desired Position",
         * leftStatus.getSeg().position);
         * SmartDashboard.putNumber("Right Position Error", leftStatus.getPosError());
         * SmartDashboard.putNumber("Right Theoretical Vel",
         * rightStatus.getSeg().velocity);
         * SmartDashboard.putNumber("Right Desired Velocity Error",
         * leftStatus.getVelError()); SmartDashboard.putNumber("Left Arb Feed",
         * leftStatus.getArbFeed()); SmartDashboard.putNumber("Right Arb Feed",
         * rightStatus.getArbFeed()); }
         */
    }
    ...
    public void updateVision() {
        //InterpolatingDouble result = Constants.kHoodAutoAimMap.getInterpolated(new InterpolatingDouble(1.0));
