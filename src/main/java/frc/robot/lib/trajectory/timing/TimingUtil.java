...
public class TimingUtil {
	...
	public static <S extends State<S>> Trajectory<TimedState<S>> timeParameterizeTrajectory(boolean reverse, final List<S> states,
		...
		for (int i = 0; i < states.size(); ++i) {
			...
			while (true) {
				...
				// vf = sqrt(vi^2 + 2*a*d)
				constraint_state.max_velocity = Math
						.min(max_velocity, Math.sqrt(predecessor.max_velocity * predecessor.max_velocity + 2.0 * predecessor.max_acceleration * ds));
				...
				for (final TimingConstraint<S> constraint : constraints) {
					final TimingConstraint.MinMaxAcceleration min_max_accel = constraint
							.getMinMaxAcceleration(constraint_state.state, (reverse ? -1.0 : 1.0) * constraint_state.max_velocity);
					if (!min_max_accel.valid()) {
						// This should never happen if constraints are well-behaved.
						throw new RuntimeException();
					}
					constraint_state.min_acceleration = Math
							.max(constraint_state.min_acceleration, reverse ? -min_max_accel.max_acceleration() : min_max_accel.min_acceleration());
					constraint_state.max_acceleration = Math
							.min(constraint_state.max_acceleration, reverse ? -min_max_accel.min_acceleration() : min_max_accel.max_acceleration());
				}
	...
	protected static class ConstrainedState<S extends State<S>> {
		...
		public String toString() {
			return state.toString() + ", distance: " + distance + ", max_velocity: " + max_velocity + ", " + "min_acceleration: "
					+ min_acceleration + ", max_acceleration: " + max_acceleration;
		}
	}
}
