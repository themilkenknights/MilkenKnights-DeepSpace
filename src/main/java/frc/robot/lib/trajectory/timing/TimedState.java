...
public class TimedState<S extends State<S>> implements State<TimedState<S>> {
	...
	public TimedState<S> interpolate(TimedState<S> other, double x) {
		...
		boolean reversing =
				velocity() < 0.0 || (Util.epsilonEquals(velocity(), 0.0) && acceleration() < 0.0);
		...
		return new TimedState<S>(
				state().interpolate(other.state(), new_s / state().distance(other.state())), new_t, new_v,
				acceleration());
	}
	...
	public String toCSV() {
		...
		return state().toCSV() + "," + fmt.format(t()) + "," + fmt.format(velocity()) + "," + fmt
				.format(acceleration());
	}
	...
	public String toString() {
		final DecimalFormat fmt = new DecimalFormat("#0.000");
		return state().toString() + ", t: " + fmt.format(t()) + ", v: " + fmt.format(velocity())
				+ ", a: " + fmt.format(acceleration());
	}
}
