...
public class DifferentialDriveDynamicsConstraint<S extends IPose2d<S> & ICurvature<S>> implements TimingConstraint<S> {
		...
		@Override public MinMaxAcceleration getMinMaxAcceleration(S state, double velocity) {
				...
				DifferentialDrive.MinMax min_max = drive_.getMinMaxAcceleration(new DifferentialDrive.ChassisState(Units.inches_to_meters(velocity), state.getCurvature() * velocity),
