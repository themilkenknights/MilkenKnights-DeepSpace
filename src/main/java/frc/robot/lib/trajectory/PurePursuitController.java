...
public class PurePursuitController<S extends ITranslation2d<S>> implements IPathFollower {
    ...
    public PurePursuitController(final DistanceView<S> path, double sampling_dist, double lookahead, double goal_tolerance) {
    ...
    public Twist2d steer(final Pose2d current_pose) {
        ...
        for (double progress = 0.0; progress <= remaining_progress; progress = Math.min(remaining_progress, progress + sampling_dist_)) {
        ...
        protected double findLength(Pose2d pose, S point, Translation2d center, double radius) {
            if (radius < Double.MAX_VALUE) {
                ...
                final boolean behind =
                    Math.signum(Translation2d.cross(pose.getRotation().normal().toTranslation(), new Translation2d(pose.getTranslation(), point.getTranslation()))) > 0.0;
