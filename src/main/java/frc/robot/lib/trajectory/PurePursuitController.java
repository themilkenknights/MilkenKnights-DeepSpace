...
public class PurePursuitController<S extends ITranslation2d<S>> implements IPathFollower {
	...
	public Twist2d steer(final Pose2d current_pose) {
		...
		for (double progress = 0.0; progress <= remaining_progress;
				progress = Math.min(remaining_progress, progress + sampling_dist_)) {
			double dist = current_pose.getTranslation()
	...
	public static class Arc<S extends ITranslation2d<S>> {
		...
		protected Translation2d findCenter(Pose2d pose, S point) {
			final Translation2d poseToPointHalfway = pose.getTranslation()
					.interpolate(point.getTranslation(), 0.5);
			final Rotation2d normal = pose.getTranslation().inverse().translateBy(poseToPointHalfway)
					.direction().normal();
		...
		protected double findLength(Pose2d pose, S point, Translation2d center, double radius) {
			if (radius < Double.MAX_VALUE) {
				...
				final boolean behind = Math.signum(Translation2d
						.cross(pose.getRotation().normal().toTranslation(),
