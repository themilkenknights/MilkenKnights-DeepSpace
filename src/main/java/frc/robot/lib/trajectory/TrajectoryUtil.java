...
public class TrajectoryUtil {
  ...
  public static <S extends IPose2d<S>> Trajectory<TimedState<S>> mirrorTimed(
    ...
    for (int i = 0; i < trajectory.length(); ++i) {
      ...
      waypoints.add(
          new TimedState<S>(timed_state.state().mirror(), timed_state.t(), timed_state.velocity(),
  ...
  public static <S extends State<S>> Trajectory<S> resample(final TrajectoryView<S> trajectory_view,
    ...
    final int num_states = (int) Math.ceil(
        (trajectory_view.last_interpolant() - trajectory_view.first_interpolant()) / interval);
    ArrayList<S> states = new ArrayList<S>(num_states);
    for (int i = 0; i < num_states; ++i) {
      states
          .add(trajectory_view.sample(i * interval + trajectory_view.first_interpolant()).state());
    }
    return new Trajectory<S>(states);
  }

  public static Trajectory<Pose2dWithCurvature> trajectoryFromPathFollower(
      IPathFollower path_follower, Pose2dWithCurvature start_state,
    ...
    while (!path_follower.isDone()) {
      ...
      final boolean curvature_valid =
          !Double.isNaN(dcurvature) && !Double.isInfinite(dcurvature) && !Double
              .isNaN(current_state.getCurvature()) && !Double
  ...
  public static Trajectory<Pose2dWithCurvature> trajectoryFromSplines(
      final List<? extends Spline> splines, double maxDx, double maxDy,
      double maxDTheta) {
