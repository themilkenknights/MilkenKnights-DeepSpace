...
public class TrajectoryUtil {
		 ...
		 */
		public static <S extends State<S>> Trajectory<S> resample(final TrajectoryView<S> trajectory_view, double interval) {
				if (interval <= Util.kEpsilon) {
						return new Trajectory<S>();
				}
				final int num_states = (int) Math.ceil((trajectory_view.last_interpolant() - trajectory_view.first_interpolant()) / interval);
		...
		public static Trajectory<Pose2dWithCurvature> trajectoryFromPathFollower(IPathFollower path_follower, Pose2dWithCurvature start_state, double step_size,
				...
				while (!path_follower.isDone()) {
						...
						if (dcurvature > dcurvature_limit && curvature_valid) {
								steering_command =
										new Twist2d(steering_command.dx, steering_command.dy, (dcurvature_limit * steering_command.norm() + current_state.getCurvature()) * steering_command.norm());
