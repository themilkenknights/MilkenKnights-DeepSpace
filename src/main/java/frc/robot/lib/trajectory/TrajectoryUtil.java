...
public class TrajectoryUtil {
	...
	public static Trajectory<Pose2dWithCurvature> trajectoryFromPathFollower(IPathFollower path_follower, Pose2dWithCurvature start_state,
		...
		while (!path_follower.isDone()) {
			...
			final Twist2d average_steering_command = !curvature_valid ? steering_command : new Twist2d(steering_command.dx, steering_command.dy,
					(current_state.getCurvature() + 0.5 * dcurvature * steering_command.norm()) * steering_command.norm());
			current_state = new Pose2dWithCurvature(current_state.getPose().transformBy(Pose2d.exp(average_steering_command)),
