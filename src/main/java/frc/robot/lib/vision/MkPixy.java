  ...
  public void pixyUpdate() {
    if (PixyResult == 0) {
      ...
      if (updateStatus > 0) {
        ...
        for (Block block : blocks) {
          if (block.getSignature() == blockSignature) {
            if (largestBlock == null) {
              largestBlock = block;
            } else if (block.getWidth() * block.getHeight() > largestBlock.getWidth()
        ...
        if (largestBlock != null) {
          mTarget = new MkPixyTarget(largestBlock.getX(),
              largestBlock.getWidth() * largestBlock.getHeight(), true, Timer.getFPGATimestamp());
        } else {
