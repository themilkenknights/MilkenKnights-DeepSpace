...
public class PixySPI {
	...
	private static final Logger logger = Logger.getLogger(PixySPI.class.getName());
	...
	public int readPackets() throws PixyException { //The signature should be which number object in
		if (debug >= 1) {
			SmartDashboard.putNumber("readPackets: count: ", readPackets++);
		}
		...
		if (numBlocks > 0) {
			if (debug >= 2) {
				logger.log(Level.INFO, "Pixy readPackets: blocks detected: {0}", Integer.toString(numBlocks));
			}
			if (debug >= 1) {
				SmartDashboard.putString("Pixy readPackets: blocks detected: ", Integer.toString(numBlocks));
			}
			...
			for (int i = 0; i < numBlocks; i++) {
				...
				if (debug >= 1) {
					SmartDashboard.putString("Pixy readPackets: Signature: " + signature, Integer.toString(signature));
				}
				if (debug >= 1) {
					SmartDashboard.putString("Pixy readPackets: " + signature + ": X: ", Integer.toString(packet.X));
				}
				if (debug >= 1) {
					SmartDashboard.putString("Pixy readPackets: " + signature + ": Y: ", Integer.toString(packet.Y));
				}
				if (debug >= 1) {
					SmartDashboard.putString("Pixy readPackets: " + signature + ": Width: ", Integer.toString(packet.Width));
				}
				if (debug >= 1) {
					SmartDashboard.putString("Pixy readPackets: " + signature + ": Height: ", Integer.toString(packet.Height));
				}
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy readPackets: Signature: " + signature, Integer.toString(signature));
				}
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy readPackets: " + signature + ": X: ", Integer.toString(packet.X));
				}
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy readPackets: " + signature + ": Y: ", Integer.toString(packet.Y));
				}
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy readPackets: " + signature + ": Width: ", Integer.toString(packet.Width));
				}
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy readPackets: " + signature + ": Height: ", Integer.toString(packet.Height));
				}

				// Add the current PixyPacket to the correct location for the signature.
				packets.get(signature).add(packet);
				if (debug >= 1) {
					SmartDashboard.putNumber("Pixy readPackets: packets size: ", packets.get(signature).size());
				}
			}
		}
	...
	private int getBlocks(int maxBlocks) {
		...
		if (debug >= 1) {
			SmartDashboard.putNumber("getBlock: count: ", getBlock++);
		}
		...
		while ((blocks.size()) < maxBlocks && (blocks.size() < PIXY_MAXIMUM_ARRAYSIZE)) {
			if (debug >= 1) {
				SmartDashboard.putNumber("getBlocks: loop count: ", count++);
			}
			...
			if (checksum == PIXY_START_WORD) {
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy: getBlocks: {0}", "checksum == PIXY_START_WORD");
				}
				skipStart = true;
				return (blocks.size());
			}
			// See if we received a empty buffer, if so, assume end of comms for now and return what we have.
				...
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy: getBlocks: {0}", "checksum == 0");
				}
				return (blocks.size());
			}
			...
			for (int i = 0; i < BLOCK_LEN; i++) {
				...
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy: getBlocks: block: {0}", intsToHex(block));
				}
				if (debug >= 1) {
					SmartDashboard.putString("Pixy: getBlocks: block", intsToHex(block));
				}
			}

			if (debug >= 2) {
				logger.log(Level.INFO, "Pixy: getBlocks checksum: {0}", Integer.toHexString(checksum));
			}
			if (debug >= 1) {
				SmartDashboard.putString("Pixy: getBlocks checksum: ", Integer.toHexString(checksum));
			}
			if (debug >= 2) {
				logger.log(Level.INFO, "Pixy: getBlocks trialsum: {0}", Integer.toHexString(trialsum));
			}
			if (debug >= 1) {
				SmartDashboard.putString("Pixy: getBlocks trialsum: ", Integer.toHexString(trialsum));
			}
			...
			if (checksum == trialsum) {
				...
				blocks.add(block);
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy: getBlocks Checksum: {0}", "passed");
				}
				if (debug >= 1) {
					SmartDashboard.putString("Pixy: getBlocks Checksum", "passed");
				}
			} else {
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy: getBlocks Checksum: {0}", "failed");
				}
				if (debug >= 1) {
					SmartDashboard.putString("Pixy: getBlocks Checksum", "failed");
				}
			}
			...
			if (debug >= 1) {
				SmartDashboard.putString("Pixy: getBlocks: w: ", (Integer.toHexString(w)));
			}
			if (debug >= 2) {
				logger.log(Level.INFO, "Pixy: getBlocks: w: ", (Integer.toHexString(w)));
			}

			if (w != PIXY_START_WORD) {
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy: getBlocks: {0}", "w != PIXY_START_WORD");
				}
				return (blocks.size());
			}
		}
	...
	private int getWord() {
		...
		if (debug >= 1) {
			SmartDashboard.putNumber("getWord: count: ", getWord++);
		}
		...
		if (debug >= 1) {
			SmartDashboard.putString("Pixy: getWord: word: ", Integer.toHexString(word));
		}
	...
	private boolean getStart() {
		...
		while (true) {
			int w = getWord();
			if (debug >= 2) {
				logger.log(Level.INFO, "Pixy: getStart: w: {0}", Integer.toHexString(w));
			}
			if (debug >= 1) {
			...
			if ((w == 0x00) && (lastw == 0x00)) {
				...
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy: getStart: {0}", "no pixy data received");
				}
				if (debug >= 1) {
					SmartDashboard.putString("Pixy: getStart", "no pixy data received");
				}
				return (false);
			} else if ((w == PIXY_START_WORD) && (lastw == PIXY_START_WORD)) {
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy: getStart: {0}", "found start");
				}
				if (debug >= 1) {
					SmartDashboard.putString("Pixy: getStart", "found start");
				}
				return (true);
			}
	...
	public void rawComms() {
		...
		while (true) {
			...
			writeString = bbToString(writeBuf);
			if (debug >= 2) {
				logger.log(Level.INFO, "Pixy: rawComms: write sync: {0}", writeString);
			}
			if (debug >= 1) {
				SmartDashboard.putString("Pixy: rawComms: write sync: ", writeString);
			}
			ret = pixy.transaction(writeBuf, readBuf, 2);
			//readBuf.flip();
			readString = bbToString(readBuf);
			if (debug >= 2) {
				logger.log(Level.INFO, "Pixy: rawComms: read sync: {0}", readString);
			}
			readBuf.rewind();
			if (debug >= 1) {
				SmartDashboard.putString("Pixy: rawComms: read sync: ", readString);
			}
			writeBuf.clear();
			readBuf.clear();
		}
