...
public class QuinticHermiteSpline extends Spline {
   ...
   */
  private QuinticHermiteSpline(double x0, double x1, double dx0, double dx1, double ddx0,
      double ddx1, double y0, double y1, double dy0, double dy1,
  ...
  private static void runOptimizationIteration(List<QuinticHermiteSpline> splines) {
    ...
    for (int i = 0; i < splines.size() - 1; ++i) {
      ...
      if (splines.get(i).getStartPose().isColinear(splines.get(i + 1).getStartPose()) || splines
          .get(i).getEndPose()
      ...
      splines.set(i,
          new QuinticHermiteSpline(temp.x0, temp.x1, temp.dx0, temp.dx1, temp.ddx0,
              temp.ddx1 + kEpsilon, temp.y0, temp.y1, temp.dy0, temp.dy1,
              temp.ddy0, temp.ddy1));
      splines.set(i + 1,
          new QuinticHermiteSpline(temp1.x0, temp1.x1, temp1.dx0, temp1.dx1, temp1.ddx0 + kEpsilon,
              temp1.ddx1, temp1.y0, temp1.y1, temp1.dy0,
              temp1.dy1, temp1.ddy0, temp1.ddy1));
      controlPoints[i].ddx = (sumDCurvature2(splines) - original) / kEpsilon;
      splines.set(i,
          new QuinticHermiteSpline(temp.x0, temp.x1, temp.dx0, temp.dx1, temp.ddx0, temp.ddx1,
              temp.y0, temp.y1, temp.dy0, temp.dy1, temp.ddy0,
              temp.ddy1 + kEpsilon));
      splines.set(i + 1,
          new QuinticHermiteSpline(temp1.x0, temp1.x1, temp1.dx0, temp1.dx1, temp1.ddx0, temp1.ddx1,
              temp1.y0, temp1.y1, temp1.dy0, temp1.dy1,
    ...
    double stepSize = fitParabola(p1, p2,
        p3); //approximate step size to minimize sumDCurvature2 along the gradient
    for (int i = 0; i < splines.size() - 1; ++i) {
      if (splines.get(i).getStartPose().isColinear(splines.get(i + 1).getStartPose()) || splines
          .get(i).getEndPose()
          .isColinear(splines.get(i + 1).getEndPose())) {
  ...
  private static double fitParabola(Translation2d p1, Translation2d p2, Translation2d p3) {
    double A = (p3.x() * (p2.y() - p1.y()) + p2.x() * (p1.y() - p3.y()) + p1.x() * (p3.y() - p2
        .y()));
