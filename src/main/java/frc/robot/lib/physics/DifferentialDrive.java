...
/**
 * Dynamic model a differential drive robot.  Note: to simplify things, this math assumes the center of mass is coincident with the
 * kinematic center of rotation (e.g. midpoint of the center axle).
 */
public class DifferentialDrive {
		...
		public WheelState solveInverseKinematics(final ChassisState chassis_motion) {
				...
				wheel_motion.left = (chassis_motion.linear - effective_wheelbase_radius_ * chassis_motion.angular) / wheel_radius_;
				wheel_motion.right = (chassis_motion.linear + effective_wheelbase_radius_ * chassis_motion.angular) / wheel_radius_;
				return wheel_motion;
		}
		...
		public void solveForwardDynamics(DriveDynamics dynamics) {
				...
				dynamics.chassis_acceleration.angular = effective_wheelbase_radius_ * (dynamics.wheel_torque.right - dynamics.wheel_torque.left) / (wheel_radius_ * moi_)
				...
				dynamics.dcurvature = (dynamics.chassis_acceleration.angular - dynamics.chassis_acceleration.linear * dynamics.curvature) / (dynamics.chassis_velocity.linear
		...
		public ChassisState solveForwardKinematics(final WheelState wheel_motion) {
				...
				chassis_motion.angular = wheel_radius_ * (wheel_motion.right - wheel_motion.left) / (2.0 * effective_wheelbase_radius_);
		...
		public void solveInverseDynamics(DriveDynamics dynamics) {
				...
				dynamics.wheel_torque.left =
						wheel_radius_ / 2.0 * (dynamics.chassis_acceleration.linear * mass_ - dynamics.chassis_acceleration.angular * moi_ / effective_wheelbase_radius_
		...
		public MinMax getMinMaxAcceleration(final ChassisState chassis_velocity, double curvature, /*double dcurvature,*/
				...
				for (boolean left : Arrays.asList(false, true)) {
						for (double sign : Arrays.asList(1.0, -1.0)) {
								...
								final double fixed_torque = fixed_transmission.getTorqueForVoltage(wheel_velocities.get(left), sign * max_abs_voltage);
								...
								if (left) {
										variable_torque =
												((/*-moi_ * chassis_velocity.linear * chassis_velocity.linear * dcurvature*/ -drag_torque) * mass_ * wheel_radius_ + fixed_torque * (linear_term
														+ angular_term)) / (linear_term - angular_term);
								} else {
										variable_torque =
												((/*moi_ * chassis_velocity.linear * chassis_velocity.linear * dcurvature*/ +drag_torque) * mass_ * wheel_radius_ + fixed_torque * (linear_term
														- angular_term)) / (linear_term + angular_term);
								...
								if (Math.abs(variable_voltage) <= max_abs_voltage + Util.kEpsilon) {
										double accel = 0.0;
										if (Double.isInfinite(curvature)) {
												accel = (left ? -1.0 : 1.0) * (fixed_torque - variable_torque) * effective_wheelbase_radius_ / (moi_ * wheel_radius_)
														- drag_torque / moi_ /*- chassis_velocity.linear * chassis_velocity.linear * dcurvature*/;
										} else {
		...
		public static class DriveDynamics implements CSVWritable {
				...
				@Override public String toCSV() {
						return curvature + "," + dcurvature + "," + chassis_velocity + ", " + chassis_acceleration + ", " + wheel_velocity + ", " + wheel_acceleration + ", " + voltage + ", "
								+ wheel_torque;
				}
		}
}
