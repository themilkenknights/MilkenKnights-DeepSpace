...
/**
 * Provides forward and inverse kinematics equations for the robot modeling the wheelbase as a differential drive (with a corrective factor
...
public class Kinematics {
  ...
  public static Twist2d forwardKinematics(double left_wheel_delta, double right_wheel_delta) {
    double delta_rotation =
        (right_wheel_delta - left_wheel_delta)
            / (Constants.DRIVE.kEffectiveDriveWheelTrackWidthInches
            * Constants.DRIVE.kTrackScrubFactor);
  ...
  public static DriveVelocity inverseKinematics(Twist2d velocity) {
    ...
    double delta_v =
        DRIVE.kEffectiveDriveWheelTrackWidthInches
            * velocity.dtheta
            / (2 * DRIVE.kTrackScrubFactor);
