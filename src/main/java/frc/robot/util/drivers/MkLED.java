...
import frc.robot.util.math.HsvToRgb;
...
public class MkLED extends CANifier {
    ...
    public void setPulse(LEDColors color1, LEDColors color2, double period) {
        ...
        if (((int) ((edu.wpi.first.wpilibj.Timer.getFPGATimestamp() - startTime) / period)) % 2
            == 0) {
            set_rgb(color1);
        } else {
            set_rgb(color2);
    ...
    public enum LEDColors {
        BLUE(new double[] {0, 0, 255}), RED(new double[] {255, 0, 0}), OFF(
            new double[] {0, 0, 0}), ORANGE(new double[] {255, 165, 0}), PURPLE(
            new double[] {128, 0, 128}), WHITE(new double[] {255, 255, 255}), GREEN(
            new double[] {0, 255, 0});
